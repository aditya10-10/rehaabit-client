...
const ShowSubCategoriesModal = ({ isOpen, setIsOpen, modalCategoryId }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
      <main className="relative flex flex-col justify-center items-center p-4 bg-white rounded-xl w-[20%] shadow-sm transform transition-transform duration-300 scale-100">
        <IoIosClose
        ...
        <div className="flex flex-col w-full items-start h-80 overflow-y-auto">
          {subCategoriesByCategory.map((subcategory) => {
            ...
            return (
              <div
                key={_id}
                className="flex items-center justify-start h-10 w-full mb-4 p-4 gap-8"
              >
